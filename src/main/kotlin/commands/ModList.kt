...
class ModList : CliktCommand(name = "modlist") {
    ...
    override fun run() = runBlocking {
        coroutineScope {
            ...
            # ${pack.getManifest().name} (${pack.getManifest().minecraft} / ${pack.getManifest().loader.type})
            
            ${pack.getManifest().summary ?: ""}
        """.trimIndent()
            ...
            val progress = progressBarContextLayout {
                ...
                terminal,
                total = pack.getManifests().size.toLong(),
                context = ""
            )

            launch { progress.execute() }

            for ((_, manifest) in pack.getManifests()) {
                progress.advance(1)
                ...
                if (manifest.sources.modrinth != null) {
                    val project =
                        ctx.modrinthApi.getProject(manifest.sources.modrinth!!.projectId)!!
                    title = project.title
                    description = project.description
                } else if (manifest.sources.curseforge != null) {
                    val project =
                        ctx.curseforgeApi.getMod(manifest.sources.curseforge!!.projectId)!!
                    title = project.name
